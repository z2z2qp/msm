    ...
    override fun preHandle(request: HttpServletRequest, response: HttpServletResponse, handler: Any): Boolean {
        ...
        if (checkVerifyCode(session, code)) {//校验验证码
            ...
            if (isLocked(ll)) {//用户是否被锁
                val str = JSON.toJSONString(Result(ControllerCode.USER_LOCKED, ll.lockTime!!), JSONWriter.Feature.WriteEnumUsingToString)
                response.writer.print(str)
            } else {
                result = true
            }
        } else {
            val str = JSON.toJSONString(Result(ControllerCode.INVALID_VERIFY_CODE), JSONWriter.Feature.WriteEnumUsingToString)
            response.writer.print(str)
        }
        return result
    }
